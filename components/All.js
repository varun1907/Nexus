...
export default class All extends React.Component {
  ...
  componentWillMount()
  {
      ...
      AsyncStorage.multiGet(['ACCESS_TOKEN' , 'ID']).then((value) => {
        if (value == null){
           alert('Login Again');
         }
         else{
           this.setState({
             ...
             return fetch('https://nexus.scriptguru.org/api/notices',{
              method: 'GET',
              headers: {
...
gotoDetailscreen=(userId,id,description,interested,going,not_interested,title)=>{
  ...
  static navigationOptions = {
    drawerIcon:(
      <Icon name="target" style={{color:'#e91e63',height:24,width:24,fontSize:25}}/>
    )
  }
      ...
      renderRow={(item) =>
        ...
        <ListItem style={{paddingTop:0,paddingBottom:0,paddingRight:0,paddingLeft:0,marginLeft:0,marginBottom:30,borderWidth:5,borderTopColor:'#673ab7'}}>
              ...
              <CardItem bordered={true}
                 button onPress={()=>{this.gotoDetailscreen(item.user_id,item.id,item.description,item.interested,item.going,item['not-interested'],item.title)}}>
...
class ButtonLike extends Component {
    ...
    render() {
        ...
        let type='going'
      return (
        <View style={[this.state.isSelected ? styles.blueLike : styles.blackLike,{justifyContent:'center',alignItems:'center',width:55,height:35}]}>
         <Button transparent style={{paddingLeft:16}}
         onPress={() => this.increase(type,id)}>
         <Icon active style={this.state.isSelected ? styles.iconBlueLike : styles.iconBlackLike} name="thumbs-up"/>
...
class ButtonInterested extends Component {
    ...
    render() {
      ...
      let type='interested'
      return (
        <View style={[this.state.isSelected ? styles.blueInterested : styles.blackInterested,{justifyContent:'center',alignItems:'center',width:55,height:35}]}>
         <Button transparent style={{paddingLeft:16}} onPress={()=>this.increase(type,id)}>
         <Icon active style={this.state.isSelected ? styles.iconBlueInterested : styles.iconBlackInterested} name="bookmark"/>
...
class ButtonNotGoing extends Component {
    ...
    render() {
      ...
      return (
        <View style={[this.state.isSelected ? styles.blueNotGoing : styles.blackNotGoing,{justifyContent:'center',alignItems:'center',width:55,height:35}]}>
         <Button transparent style={{paddingLeft:16}} onPress={()=>{this.setState({isSelected: !this.state.isSelected}); }}>
         <Icon active style={this.state.isSelected ? styles.iconBlueNotGoing : styles.iconBlackNotGoing} name="thumbs-down"/>
